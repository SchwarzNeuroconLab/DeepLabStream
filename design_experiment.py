...
def design_experiment(default):
    ...
    input_dict = dict(EXPERIMENT=None, TRIGGER=None, PROCESS=None, STIMULATION=None)

    def get_input(input_name):
        click.echo(
            f"Choosing {input_name}... \n Available {input_name}s are: "
            + ", ".join(config.get_available_module_names(input_name))
        )
    ...
    if input_value == "BaseOptogeneticExperiment":
        ...
        click.echo(
            f"Available Triggers are: "
            + ", ".join(config.get_available_module_names("TRIGGER"))
        )
        click.echo(
            "Note, that you cannot select the same Trigger as selected in TRIGGER."
        )

        input_value = click.prompt(
            f"Enter TRIAL_TRIGGER for BaseTrialExperiment", type=str
        )
    ...
    if input_dict["EXPERIMENT"] == "BaseTrialExperiment":
        ...
        input_dict["STIMULATION"] == "ScreenStimulation"
        or input_dict["STIMULATION"] == "RewardDispenser"
    ):
        input_dict["PROCESS_TYPE"] = "supply"

    if input_dict["EXPERIMENT"] == "BaseTrialExperiment":
        config.import_default(
            experiment_name=input_dict["EXPERIMENT"],
            trigger_name=input_dict["TRIGGER"],
            process_name=input_dict["PROCESS"],
            stimulation_name=input_dict["STIMULATION"],
    ...
    if "PROCESS_TYPE" in input_dict.keys():
        config._change_parameter(
            module_name=input_dict["PROCESS"],
            parameter_name="TYPE",
            parameter_value=input_dict["PROCESS_TYPE"],
        )

    if click.confirm(
        "Do you want to set parameters as well (Not recommended)? \n Note, that you can change them in the created file later."
    ):
                ...
                if module not in ignore_list:
                    for input_key in parameter_dict.keys():
                        if input_key not in inner_ignore_list:
                            click.echo(
                                f"Default {input_key} is: "
                                + str(parameter_dict[input_key])
                            )
                            input_value = click.prompt(f"Enter new value: ", type=str)
                            config._change_parameter(
                                module_name=module,
                                parameter_name=input_key,
                                parameter_value=input_value,
                            )
        except:
            click.echo(
                "Failed to set individual parameters. Please change them later in the config file..."
            )
    else:
        click.echo(
            "Skipping parameters. Experiment config will be created with default values..."
        )
    """Finish up"""
